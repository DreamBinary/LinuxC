...
/* 在笔记本后面追加几个页面 */
    ...
    g_signal_connect_swapped(G_OBJECT(button), "clicked", G_CALLBACK(gtk_notebook_next_page), notebook);
    gtk_table_attach_defaults(GTK_TABLE(table), button, 1, 2, 1, 2);
    gtk_widget_show(button);
    button = gtk_button_new_with_label("prev ");
    g_signal_connect_swapped(G_OBJECT(button), "clicked", G_CALLBACK(gtk_notebook_prev_page), notebook);
